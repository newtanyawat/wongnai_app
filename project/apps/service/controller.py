...
def service_upload(cursor):
    try:
        ...
        create_at = datetime.now()  #วันสร้างไฟล์ ไม่  +7
        ...
        if request.files :
            ...
            VALUES (?,?,?,?,?,?,?,?)"""
            db_files = cursor.execute(into_files ,(uuid_file , file_name , uuid_name , password ,int(download) , session["user_login"] , dateNOW ,exp_date ))
